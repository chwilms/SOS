...
def process_image(path, num_samples, prune_size, min_score):
    ...
    object_prior = gaussian(object_prior, 20)
...
def launch(args):
    edge_images = glob.glob(os.path.join(args.edge_images,'*.png'))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Script for generating prompts based on Contour object prior.')
    parser.add_argument('edge_images', type=str, required=True,
                        help="Path to edge image directory, e.g., generated from COCO's train2017 images.")
